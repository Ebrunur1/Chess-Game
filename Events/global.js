...
function moveElement(piece, id, castle) {
  const from = piece?.current_position ?? null;
  const to = id;
  const destBefore = globalState.flat().find((el) => el.id == id);
  const capturedPieceName =
    destBefore && destBefore.piece ? destBefore.piece.piece_name : null;
    ...
    window.dispatchEvent(
      new CustomEvent("pieceMove", {
        detail: {
          from,
          to,
          piece: piece?.piece_name ?? null,
          captured: capturedPieceName,
          castle: !!castle,
        },
      })
    );
